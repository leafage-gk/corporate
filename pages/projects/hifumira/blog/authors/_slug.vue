<template>
  <v-content>
    <blog-hero-header :subtitle="author.name" />
    <v-container class="m-plus-rounded-1c">
      <v-breadcrumbs :items="items" divider=">"></v-breadcrumbs>
      <v-layout column>
        <v-subheader>
          <h2>執筆者 - {{ author.name }}</h2>
        </v-subheader>
        <no-ssr>
...
import { BlogPostSummary, BlogAuthor } from '~/domains/contentful';
...
export default Vue.extend<Data, {}, {}, never>({
  ...
  async asyncData(context: Context) {
    ...
    const author = await context.$blog.author.get(slug);
    if (author) {
      const posts = await context.$blog.fetchByAuthor(author.id, 0, 10);
      return {
        posts,
        author,
        items: [
  ...
  head() {
    return {
      title: `執筆者 - ${this.author.name} - VTuberプロジェクト＊ひふみら`,
    };
  },
});
</script>
